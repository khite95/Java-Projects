...
public class MoverGUI extends JPanel implements NewFrame 
{
    ...
    private void handleButtonEvent( int buttonId )
    {
        ...
        float floatX;
        float floatY;
        float intX = _snowman.getMoveX( );
        float intY = _snowman.getMoveY( );
        
        switch( buttonId )
        {
            case 0:
                _timer.start( );
                System.out.println( "Button ID: " + buttonId + " - Start" );
                break;
            case 1:
                _timer.stop( );
                System.out.println( "Button ID: " + buttonId + " - Stop" );
                break;
            case 2:
                intX = _snowman.getMoveX( );
                intY = _snowman.getMoveY( );
                floatX = ( float ) intX * MoverGUI.fasterRatio;
                floatY = ( float ) intY * MoverGUI.fasterRatio;
                intX = Math.round( floatX );
                intY = Math.round( floatY );
                _snowman.setMove( intX, intY );
                System.out.println( "Button ID: " + buttonId + " - Faster "
                                  + "Increase: " + intX + " " + intY );
                break;
            case 3:
                if( intX >= MoverGUI.minStepSize 
                       && intY >= MoverGUI.minStepSize )
                {   intX = _snowman.getMoveX( );
                    intY = _snowman.getMoveY( );
                    floatX = ( float ) intX * MoverGUI.slowerRatio;
                    floatY = ( float ) intY * MoverGUI.slowerRatio;
                    intX = Math.round( floatX );
                    intY = Math.round( floatY );
                    _snowman.setMove( intX, intY );
                }
                System.out.println( "Button ID: " + buttonId + " - Slower" 
                                  + "Decrease: " + intX + " " + intY );
                break;
            case 4:
                System.out.println( "Button ID: " + buttonId + " - Reset" );
                break;
        }
    }
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
